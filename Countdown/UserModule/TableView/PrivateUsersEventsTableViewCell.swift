...
protocol PrivateUsersEventsTableViewCellDelegate: AnyObject {
    func valueChanged(_ value: Bool)
}

class PrivateUsersEventsTableViewCell: RoundedTableViewCell {

    //MARK: - Naming
    
    weak var delegate: PrivateUsersEventsTableViewCellDelegate?
    
    static let identifier = "PrivateUsersEventsTableViewCell"
    
    private lazy var label: UILabel = {
        let label = UILabel()
        label.translatesAutoresizingMaskIntoConstraints = false
        label.text = "Private Events"
    ...
    func configure(isPrivateAvailable: Bool, position: CellPosition) {
        super.configure(position: position)
        self.privateEventsSwitch.isOn = isPrivateAvailable
    ...
    @objc func valueChanged() {
        delegate?.valueChanged(privateEventsSwitch.isOn)
    }
}
